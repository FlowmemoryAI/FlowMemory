...
import {
  ...
  flowmemoryAccountStateRoot,
  flowmemoryBlockHash,
  flowmemoryBridgeCreditId,
  flowmemoryBridgeEvidenceHash,
  flowmemoryBridgeObservationId,
  flowmemoryDexStateRoot,
  flowmemoryEventRoot,
  flowmemoryFinalityReceiptId,
  flowmemoryReceiptRoot,
  flowmemoryTokenStateRoot,
  flowmemoryTransactionId,
  flowmemoryTxRoot,
  flowmemoryWithdrawalIntentId,
...
import { verifyFlowMemoryEnvelope } from "./runtime-validation.js";
...
export async function buildProductionNetworkVectors() {
  const accounts = {
    user: account("user", 1),
  ...
  const assets = {
    native: keccakUtf8("flowmemory.asset.native-test-unit.v0"),
    token: keccakUtf8("flowmemory.asset.token.launch-demo.v0")
  };
  const poolId = keccakUtf8("flowmemory.pool.native-token.demo");
  ...
  const bridgeObservationId = flowmemoryBridgeObservationId(bridgeSourceEvent);
  const canonicalBridgeCreditId = flowmemoryBridgeCreditId({
  ...
  const documents = {
    ...
    tokenLaunch: productTokenLaunchDocument({
      ...
      nameHash: keccakUtf8("FlowMemory Test Token"),
    ...
    withdrawalIntent: bridgeWithdrawalIntentDocument({
      ...
      amount: "10000000",
      flowmemoryAccount: accounts.user.accountId,
  ...
  return {
    schema: "flowmemory.crypto.production-network-vectors.v0",
    chainId: CHAIN_ID,
    networkProfile: NETWORK_PROFILE,
    issuedAtUnixMs: ISSUED_AT_UNIX_MS,
    expiresAtUnixMs: EXPIRES_AT_UNIX_MS,
    boundary: "Deterministic local/private production-network-shaped crypto vectors. Fixtures contain public test metadata, documents, signatures, and expected validation failures only.",
...
function account(role, index) {
  ...
  return {
    ...
    accountId: flowmemoryAccountId({ publicKey, role }),
    signerKeyId: flowmemorySignerKeyId({ publicKey }),
    publicMetadata
  };
}
...
async function signedVector(name, document, signer, nonce, payloadType) {
  ...
  envelope.transactionId = flowmemoryTransactionId(envelope);
  const runtime = verifyFlowMemoryEnvelope({
    document,
    envelope,
...
function buildHashHelperVectors({
  ...
  const txRoot = flowmemoryTxRoot(positives.map((entry) => entry.envelope.transactionId));
  const receiptRoot = flowmemoryReceiptRoot(positives.map((entry) => entry.expected.payloadHash));
  const eventRoot = flowmemoryEventRoot([bridgeObservationId, canonicalBridgeCreditId]);
  const accountStateRoot = flowmemoryAccountStateRoot(Object.values(accounts).map((entry) => entry.publicMetadata));
  const tokenStateRoot = flowmemoryTokenStateRoot([assets.native, assets.token]);
  const dexStateRoot = flowmemoryDexStateRoot([{ poolId, baseAssetId: assets.native, quoteAssetId: assets.token }]);
  const blockHash = flowmemoryBlockHash({
    chainId: CHAIN_ID,
    networkProfile: NETWORK_PROFILE,
    blockNumber: "9",
  ...
  const finalityInput = {
    ...
    round: "1",
    voteRoot: flowmemoryReceiptRoot([accounts.validator.accountId, blockHash])
  };
  const bridgeSourceEventKey = bridgeSourceEventReplayKey(bridgeSourceEvent);
  const bridgeEvidenceHash = flowmemoryBridgeEvidenceHash({
  ...
  return {
    ...
    bridgeEvidenceHash,
    withdrawalIntentId: flowmemoryWithdrawalIntentId(withdrawalIntentInput),
    finalityReceiptId: flowmemoryFinalityReceiptId(finalityInput),
    replayKeys: {
...
function buildNegativeVectors({ positives, accounts, bridgeSourceEvent }) {
  ...
  const specs = [
    ...
    },
    {
      name: "wrong-domain",
      base: "wallet-transfer",
      envelope: { domain: "flowmemory.production-network.v0.transaction-envelope:profile:private-lan:chain:31337" },
  ...
  return specs.map((spec) => {
    ...
    const result = verifyFlowMemoryEnvelope({ document, envelope, context });
    return {
      name: `production-network.${spec.name}`,
      base: spec.base,
      mutation: {
...
function productTransferDocument(input) {
  return { schema: "flowmemory.product_transfer.v0", transferId: productTransferId(input), ...input };
}

function productTokenLaunchDocument(input) {
  return { schema: "flowmemory.product_token_launch.v0", tokenLaunchId: productTokenLaunchId(input), ...input };
}

function productPoolCreateDocument(input) {
  return { schema: "flowmemory.product_pool_create.v0", poolCreateId: productPoolCreateId(input), ...input };
}

function productAddLiquidityDocument(input) {
  return { schema: "flowmemory.product_add_liquidity.v0", addLiquidityId: productAddLiquidityId(input), ...input };
}

function productRemoveLiquidityDocument(input) {
  return { schema: "flowmemory.product_remove_liquidity.v0", removeLiquidityId: productRemoveLiquidityId(input), ...input };
}

function productSwapDocument(input) {
  return { schema: "flowmemory.product_swap.v0", swapId: productSwapId(input), ...input };
}

function localBalanceRecordDocument(input) {
  return { schema: "flowmemory.local_balance_record.v0", balanceRecordId: localBalanceRecordId(input), ...input };
}

function bridgeCreditDocument(input) {
  const creditId = bridgeCreditId({ ...input, status: input.statusCode });
  return { schema: "flowmemory.bridge_credit.v0", creditId, ...input };
}
...
function finalityReceiptDocument(input) {
  ...
  return { schema: "flowmemory.finality_receipt.v0", finalityReceiptId: finalityReceiptId(idInput), ...input };
}
