...
const defaultFixturePath = resolve(import.meta.dirname, "..", "fixtures", "production-network-vectors.json");

export function validateProductionNetworkCrypto(fixturePath = defaultFixturePath) {
  const fixture = readJson(fixturePath);
  assert.equal(fixture.schema, "flowmemory.crypto.production-network-vectors.v0");
  ...
  for (const vector of fixture.positive) {
    ...
    assert.equal(flowmemoryTransactionId(vector.envelope), vector.expected.transactionId, `${vector.name} tx id`);

    const result = verifyFlowMemoryEnvelope({
...
function documentSchemaPath(schema) {
  if (schema.startsWith("flowmemory.product_")) {
    return "../../schemas/flowmemory/product-transaction.schema.json";
  }
  const bySchema = {
    "flowmemory.local_balance_record.v0": "../../schemas/flowmemory/local-balance-record.schema.json",
    "flowmemory.bridge_credit.v0": "../../schemas/flowmemory/bridge-credit.schema.json",
    "flowmemory.bridge_withdrawal_intent.v0": "../../schemas/flowmemory/bridge-withdrawal-intent.schema.json",
    "flowmemory.finality_receipt.v0": "../../schemas/flowmemory/finality-receipt.schema.json"
  };
...
function assertNoSecrets(value) {
  ...
  assert.doesNotMatch(
    ...
    "production-network crypto fixture contains secret-shaped material"
  );
  assert.doesNotMatch(
    serialized,
    /https:\/\/hooks\.slack\.com|https:\/\/discord\.com\/api\/webhooks/i,
    "production-network crypto fixture contains webhook-shaped material"
  );
}
...
if (fileURLToPath(import.meta.url) === resolve(process.argv[1])) {
  const result = validateProductionNetworkCrypto(process.argv[2]);
  console.log(
    `FLOWMEMORY_PRODUCTION_NETWORK_CRYPTO_OK positive=${result.positive} negative=${result.negative} hashHelpers=${result.hashHelpers} schemas=${result.schemas}`
  );
}
