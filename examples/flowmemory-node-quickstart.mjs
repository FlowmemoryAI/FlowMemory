import { FlowMemoryClient, redactFlowMemoryText } from "../services/flowmemory-sdk/src/index.ts";
...
async function main() {
  ...
  const timeoutMs = Number.parseInt(process.env.FLOWMEMORY_RPC_TIMEOUT_MS ?? "30000", 10);
  ...
  if (shouldSend) {
    ...
    walletSend = await client.walletSend({
      ...
      amountUnits: "1",
      memo: `flowmemory-node-example-${Date.now()}`,
      applyBlock: true,
  ...
  console.log(JSON.stringify({
    schema: "flowmemory.example.node_quickstart.v0",
    status: "passed",
    rpcEndpoint: redactFlowMemoryText(rpcUrl),
...
main().catch((error) => {
  console.error(error instanceof Error ? redactFlowMemoryText(error.message) : redactFlowMemoryText(String(error)));
  process.exitCode = 1;
});
