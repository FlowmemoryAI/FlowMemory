...
contract SettlementSubmitter {
    function commitObject(
        FlowMemorySettlementSpine spine,
        bytes32 objectType,
        bytes32 objectId,
        bytes32 rootfieldId,
        bytes32 commitment,
    ...
    function setSubmitterAuthorization(FlowMemorySettlementSpine spine, address submitter, bool authorized) external {
        spine.setSubmitterAuthorization(submitter, authorized);
    }
}

contract FlowMemorySettlementSpineTest {
    ...
    bytes32 private constant OBJECT_COMMITTED_SIGNATURE =
        keccak256("FlowMemoryObjectCommitted(bytes32,bytes32,bytes32,address,bytes32,bytes32,uint64,uint64,string)");

    FlowMemorySettlementSpine private spine;
    SettlementSubmitter private submitter;

    error AssertionFailed();

    function setUp() public {
        spine = new FlowMemorySettlementSpine(address(this));
        submitter = new SettlementSubmitter();
    }

    function testConstructorRequiresOwnerAndAuthorizesOwner() public {
        vm.expectRevert(FlowMemorySettlementSpine.ZeroOwner.selector);
        new FlowMemorySettlementSpine(address(0));
    ...
    function testNonOwnerCannotAuthorizeSubmitter() public {
        vm.expectRevert(abi.encodeWithSelector(FlowMemorySettlementSpine.NotOwner.selector, address(submitter)));
    ...
    function testCommitBridgeDepositObjectEmitsStableEventAndStoresRecord() public {
        ...
        FlowMemorySettlementSpine.ObjectCommitment memory record = spine.getObjectCommitment(objectId);
    ...
    function testCommitBridgeAndControlPlaneObjectVocabularyWithStableEventShape() public {
        bytes32 rootfieldId = keccak256("rootfield.flowmemory.objects");
    ...
    function testAuthorizedSubmitterCanCommitAndRevocationBlocksFutureCommits() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(FlowMemorySettlementSpine.SubmitterNotAuthorized.selector, address(submitter))
        );
    ...
    function testCommitRejectsUnauthorizedZeroFieldsAndDuplicates() public {
        ...
        vm.expectRevert(FlowMemorySettlementSpine.ZeroObjectType.selector);
        spine.commitObject(
            bytes32(0),
        ...
        vm.expectRevert(FlowMemorySettlementSpine.ZeroObjectId.selector);
        spine.commitObject(
            bridgeDepositObject,
            bytes32(0),
            keccak256("rootfield.bridge"),
        ...
        vm.expectRevert(FlowMemorySettlementSpine.ZeroRootfieldId.selector);
        spine.commitObject(
            bridgeDepositObject,
        ...
        vm.expectRevert(FlowMemorySettlementSpine.ZeroCommitment.selector);
        spine.commitObject(
            bridgeDepositObject,
        ...
        vm.expectRevert(abi.encodeWithSelector(FlowMemorySettlementSpine.ObjectAlreadyCommitted.selector, objectId));
        spine.commitObject(
            bridgeDepositObject,
            objectId,
            keccak256("rootfield.bridge"),
    ...
    function testMissingObjectLookupReverts() public {
        ...
        _assertTrue(!spine.isObjectCommitted(objectId));
        vm.expectRevert(abi.encodeWithSelector(FlowMemorySettlementSpine.ObjectNotCommitted.selector, objectId));
