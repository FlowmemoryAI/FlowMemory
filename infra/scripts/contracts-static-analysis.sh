...
if [ "$REQUIRE_SLITHER" = "1" ] && command -v slither >/dev/null 2>&1; then
  slither . --config-file .slither.config.json
elif [ "$REQUIRE_SLITHER" = "1" ]; then
  echo "slither is required but was not found on PATH" >&2
  exit 1
else
  echo "warning: slither is optional for this default gate; run npm run contracts:hardening:slither or set REQUIRE_SLITHER=1 in audit environments" >&2
fi
