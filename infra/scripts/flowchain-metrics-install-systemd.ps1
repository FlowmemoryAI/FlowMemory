param(
    ...
    [string] $Action = "Plan",
    [string] $ServiceName = "flowchain-ops-metrics.service",
    [string] $TimerName = "flowchain-ops-metrics.timer",
    ...
    [string] $ReportPath = "docs/agent-runs/live-product-infra-rpc/metrics-install-systemd-report.json",
    [string] $MarkdownPath = "docs/agent-runs/live-product-infra-rpc/SYSTEMD_METRICS_INSTALL.md"
)
...
function Test-MetricsSystemdWindowsHost {
    return [System.Runtime.InteropServices.RuntimeInformation]::IsOSPlatform([System.Runtime.InteropServices.OSPlatform]::Windows)
}

function Test-MetricsSystemdUnitName {
    param(
...
function Get-MetricsSystemdTool {
...
function ConvertTo-MetricsSystemdSafeLine {
...
function Invoke-MetricsSystemdHostCommand {
    param(
    ...
    $exitCode = 1
    try {
        $output = @(& $FilePath @ArgumentList 2>&1 | ForEach-Object { ConvertTo-MetricsSystemdSafeLine -Line $_ })
        $exitCode = if ($null -ne $LASTEXITCODE) { [int]$LASTEXITCODE } else { 0 }
    }
    catch {
        $output = @($_.Exception.Message | ForEach-Object { ConvertTo-MetricsSystemdSafeLine -Line $_ })
        $exitCode = 1
    }
...
function Get-MetricsSystemdUnitSnapshot {
    param(
    ...
    $active = Invoke-MetricsSystemdHostCommand -FilePath $SystemctlPath -ArgumentList @("is-active", $Name)
    $enabled = Invoke-MetricsSystemdHostCommand -FilePath $SystemctlPath -ArgumentList @("is-enabled", $Name)
    return [ordered]@{
        name = $Name
        canQuery = $true
...
function ConvertTo-MetricsSystemdArg {
...
function ConvertTo-MetricsSystemdCommandLine {
    param([Parameter(Mandatory = $true)][string[]] $Arguments)

    return ($Arguments | ForEach-Object { ConvertTo-MetricsSystemdArg -Value $_ }) -join " "
}

function Test-MetricsTextHasAll {
    param(
        [Parameter(Mandatory = $true)][string] $Text,
...
if (-not (Test-MetricsSystemdUnitName -Name $ServiceName -Suffix "service")) {
    throw "ServiceName must be a systemd .service unit name."
}
if (-not (Test-MetricsSystemdUnitName -Name $TimerName -Suffix "timer")) {
...
$unitMetricsScriptPath = ($metricsScriptPath -replace [System.Text.RegularExpressions.Regex]::Escape($repoRoot), $unitRepoRoot)
$readWritePaths = @(
    "$unitRepoRoot/devnet",
...
$execStart = "$PowerShellCommand $(ConvertTo-MetricsSystemdCommandLine -Arguments $execArguments)"
...
$serviceUnitText = @(
    "[Unit]",
    "Description=FlowChain ops metrics export",
...
$timerUnitText = @(
    "[Unit]",
    "Description=Run FlowChain ops metrics export every $IntervalMinutes minutes",
    "",
    "[Timer]",
    "OnBootSec=2min",
...
$isWindowsHost = Test-MetricsSystemdWindowsHost
$systemctl = Get-MetricsSystemdTool -Name "systemctl"
$journalctl = Get-MetricsSystemdTool -Name "journalctl"
...
$before = [ordered]@{
    service = Get-MetricsSystemdUnitSnapshot -SystemctlPath $systemctl.source -Name $ServiceName -CanQuery $canQuerySystemd
    timer = Get-MetricsSystemdUnitSnapshot -SystemctlPath $systemctl.source -Name $TimerName -CanQuery $canQuerySystemd
}
...
try {
    switch ($Action) {
        "Install" {
            ...
            [void]$mutationCommands.Add("write-systemd-metrics-units")
            foreach ($arguments in @(
                    @("daemon-reload"),
                    @("enable", "--now", $TimerName)
                )) {
                $result = Invoke-MetricsSystemdHostCommand -FilePath $systemctl.source -ArgumentList $arguments
        ...
        "Uninstall" {
            ...
            $disable = Invoke-MetricsSystemdHostCommand -FilePath $systemctl.source -ArgumentList @("disable", "--now", $TimerName)
            ...
            $hostMutationPerformed = $true
            [void]$mutationCommands.Add("remove-systemd-metrics-units")
            $reload = Invoke-MetricsSystemdHostCommand -FilePath $systemctl.source -ArgumentList @("daemon-reload")
...
$checks = [ordered]@{
    unitNamesValid = (Test-MetricsSystemdUnitName -Name $ServiceName -Suffix "service") -and (Test-MetricsSystemdUnitName -Name $TimerName -Suffix "timer")
    ...
    serviceUnitIncludesMetricsScript = $serviceUnitText.Contains("flowchain-ops-metrics-export.ps1")
    ...
    serviceUnitHasMetricsJsonPath = $serviceUnitText.Contains("-MetricsJsonPath")
    serviceUnitHasPrometheusTextPath = $serviceUnitText.Contains("-PrometheusTextPath")
    serviceUnitHasNoExternalDelivery = $true
    serviceUnitOwnerEnvFileInjectable = if ($ownerEnvConfigured) { Test-MetricsTextHasAll -Text $serviceUnitText -Tokens @("EnvironmentFile=-", "FLOWCHAIN_OWNER_ENV_FILE=") } else { $true }
    serviceUnitHardeningPresent = Test-MetricsTextHasAll -Text $serviceUnitText -Tokens @("NoNewPrivileges=true", "PrivateTmp=true", "ProtectSystem=full")
    serviceUnitWritePathsScoped = Test-MetricsTextHasAll -Text $serviceUnitText -Tokens @("/devnet", "/docs/agent-runs", "/services/bridge-relayer/out")
...
$requiredForPlan = @(
    ...
    "serviceUnitIncludesMetricsScript",
    ...
    "serviceUnitHasMetricsJsonPath",
    "serviceUnitHasPrometheusTextPath",
...
$report = [ordered]@{
    schema = "flowchain.metrics_install_systemd_report.v0"
...
$secretMarkerFindings = @(Get-MetricsSystemdSecretMarkerFindings -Text $preliminaryText -Label "systemd metrics install report")
...
Assert-FlowChainNoSecretText -Text $reportText -Label "systemd metrics install report"
Write-FlowChainJson -Path $reportFullPath -Value $report -Depth 20

$markdownLines = New-Object System.Collections.Generic.List[string]
$markdownLines.Add("# FlowChain Systemd Metrics Install")
$markdownLines.Add("")
$markdownLines.Add("Generated: $($report.generatedAt)")
$markdownLines.Add("Status: $status")
$markdownLines.Add("Action: $Action")
$markdownLines.Add("")
$markdownLines.Add("This script installs, checks, or removes a Linux systemd timer that refreshes no-secret ops metrics JSON and Prometheus textfile outputs on a fixed interval. It writes local metrics only and does not store external delivery credentials.")
$markdownLines.Add("")
...
Assert-FlowChainNoSecretText -Text $markdownText -Label "systemd metrics install markdown"
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $markdownFullPath) | Out-Null
Set-Content -LiteralPath $markdownFullPath -Value $markdownText -Encoding UTF8

Write-Host "FlowChain systemd metrics install status: $status"
