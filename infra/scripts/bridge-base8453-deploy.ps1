param(
    ...
    [string]$ReportPath = "services/bridge-relayer/out/base8453-deploy-readiness.json"
)
...
if ($Mode -eq "DryRun" -and $missing.Count -gt 0) {
    Write-JsonReport -Value ([ordered]@{
        ...
        mode = $Mode
        status = "missing-env-safe"
        missingEnvNames = $missing
        requiredEnvNames = $requiredEnvNames
        exactDryRunCommand = "npm run bridge:deploy:dry-run"
        exactBroadcastCommand = "npm run bridge:deploy:base8453 -- -AcknowledgeBroadcast"
        noSecrets = $true
    })
    Write-Host "Dry-run deploy readiness is safely blocked on missing env names."
    return
}
...
$report = [ordered]@{
    ...
    maxDepositWei = $maxDeposit
    totalCapWei = $totalCap
