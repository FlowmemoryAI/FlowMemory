...
const PROTOCOL_VERSION = "flowmemory.private_local_l1.protocol.v0";
const GENESIS_INPUT_SCHEMA = "flowmemory.production_l1.genesis_input.v0";
const GENESIS_SCHEMA = "flowmemory.production_l1.genesis.v0";
const PROFILE_SCHEMA = "flowmemory.production_l1.network_profile.v0";
const ACCOUNT_SCHEMA = "flowmemory.production_l1.account_public_metadata.v0";
const AUTHORITY_SCHEMA = "flowmemory.production_l1.validator_authority.v0";
const PAYLOAD_SCHEMA = "flowmemory.production_l1.transaction_payload.v0";
const ENVELOPE_SCHEMA = "flowmemory.production_l1.transaction_envelope.v0";
const BLOCK_HEADER_SCHEMA = "flowmemory.production_l1.block_header.v0";
const BLOCK_BODY_SCHEMA = "flowmemory.production_l1.block_body.v0";
const RECEIPT_SCHEMA = "flowmemory.production_l1.receipt.v0";
const EVENT_SCHEMA = "flowmemory.production_l1.event.v0";
const STATE_ROOT_SCHEMA = "flowmemory.production_l1.state_root_manifest.v0";
const BRIDGE_EVIDENCE_SCHEMA = "flowmemory.production_l1.bridge_evidence.v0";
const FINALITY_SCHEMA = "flowmemory.production_l1.finality_receipt.v0";
const EXPORT_SNAPSHOT_SCHEMA = "flowmemory.production_l1.export_snapshot.v0";

const PROFILE_IDS = [
  "flowmemory-local-private",
  "flowmemory-local-multinode",
  "flowmemory-base8453-pilot"
];
...
function buildSchemas() {
  ...
  schemas["production-network-profile.schema.json"] = schemaDoc("production-network-profile.schema.json", "FlowMemory Private/Local Network Profile V0", {
    type: "object",
    ...
    properties: {
      ...
      legacyAliases: prop("protocol", { type: "array", items: { enum: ["flowmemory-local", "flowmemory-private-lan"] }, uniqueItems: true }),
      ...
      genesisHashRules: prop("crypto", {
        ...
        properties: {
          ...
          domain: prop("crypto", { const: "flowmemory.production_l1.genesis_hash.v0" }),
  ...
  schemas["production-account-public-metadata.schema.json"] = schemaDoc("production-account-public-metadata.schema.json", "FlowMemory Account Public Metadata V0", {
    type: "object",
    additionalProperties: false,
    required: [
      "schema",
  ...
  schemas["production-validator-authority.schema.json"] = schemaDoc("production-validator-authority.schema.json", "FlowMemory Validator Authority V0", {
    type: "object",
    additionalProperties: false,
    required: [
      "schema",
  ...
  schemas["production-genesis.schema.json"] = schemaDoc("production-genesis.schema.json", "FlowMemory Genesis V0", {
    type: "object",
    additionalProperties: false,
    required: [
      "schema",
    ...
    properties: {
      ...
      bridgePilotConfig: prop("bridge", {
        ...
        properties: {
          ...
          destinationProfile: prop("bridge", { const: "flowmemory-base8453-pilot" }),
      ...
      genesisHashInput: prop("crypto", {
        ...
        properties: {
          schema: prop("crypto", { const: "flowmemory.production_l1.genesis_hash_input.v0" }),
...
function buildPayloadSchema(commonDefs) {
  ...
  return schemaDoc("production-transaction-payload.schema.json", "FlowMemory Transaction Payload Union V0", {
...
function buildEnvelopeSchema(commonDefs) {
  return schemaDoc("production-transaction-envelope.schema.json", "FlowMemory Transaction Envelope V0", {
    type: "object",
    additionalProperties: false,
    required: [
      "schema",
...
function buildEventSchema(commonDefs) {
  return schemaDoc("production-event.schema.json", "FlowMemory Event V0", {
    type: "object",
    additionalProperties: false,
...
function buildReceiptSchema(commonDefs) {
  return schemaDoc("production-receipt.schema.json", "FlowMemory Receipt V0", {
    type: "object",
    additionalProperties: false,
...
function buildBlockHeaderSchema(commonDefs) {
  return schemaDoc("production-block-header.schema.json", "FlowMemory Block Header V0", {
    type: "object",
    additionalProperties: false,
...
function buildBridgeEvidenceSchema(commonDefs) {
  return schemaDoc("production-bridge-evidence.schema.json", "FlowMemory Bridge Evidence V0", {
    type: "object",
    additionalProperties: false,
...
function buildStateRootSchema(commonDefs) {
  return schemaDoc("production-state-root-manifest.schema.json", "FlowMemory State Root Manifest V0", {
    type: "object",
    additionalProperties: false,
...
function buildFinalitySchema(commonDefs) {
  return schemaDoc("production-finality-receipt.schema.json", "FlowMemory Finality Receipt V0", {
    type: "object",
    additionalProperties: false,
...
function buildBlockBodySchema(commonDefs) {
  return schemaDoc("production-block-body.schema.json", "FlowMemory Block Body V0", {
    type: "object",
    additionalProperties: false,
...
function buildExportSnapshotSchema(commonDefs) {
  return schemaDoc("production-export-snapshot.schema.json", "FlowMemory Export Snapshot V0", {
    type: "object",
    additionalProperties: false,
...
function buildProfiles() {
  ...
  const base = {
    ...
    genesisHashRules: {
      ...
      domain: "flowmemory.production_l1.genesis_hash.v0",
  ...
  return [
    {
      ...base,
      profileId: "flowmemory-local-private",
      legacyAliases: ["flowmemory-local"],
      chainId: "742001",
      networkName: "FlowMemory Local Private",
      ...
      defaultDataDirectory: "local-runtime/local/flowmemory-local-private"
    },
    {
      ...base,
      profileId: "flowmemory-local-multinode",
      legacyAliases: ["flowmemory-private-lan"],
      chainId: "742002",
      networkName: "FlowMemory Local Multinode",
      ...
      defaultDataDirectory: "local-runtime/local/flowmemory-local-multinode"
    },
    {
      ...base,
      profileId: "flowmemory-base8453-pilot",
      legacyAliases: [],
      chainId: "7428453",
      networkName: "FlowMemory Base 8453 Pilot Destination",
      ...
      blockTimeTargetMs: 2000,
      defaultDataDirectory: "local-runtime/local/flowmemory-base8453-pilot"
    }
  ];
}
...
function buildAll() {
  ...
  const profile = profiles.find((entry) => entry.profileId === "flowmemory-base8453-pilot");
  const accounts = buildAccounts(profile);
  const accountByLabel = Object.fromEntries(accounts.map((account) => [account.label, account]));
  const nativeAssetId = hashJson("flowmemory.production_l1.asset_id.v0", { symbol: "FLOW-LOCAL", profileId: profile.profileId });
  const tokenId = hashJson("flowmemory.production_l1.token_id.v0", { symbol: "FLOWT", profileId: profile.profileId });
  const poolId = hashJson("flowmemory.production_l1.pool_id.v0", { baseAssetId: nativeAssetId, quoteAssetId: tokenId });
  ...
  const bridgePilotConfig = {
    ...
    destinationProfile: "flowmemory-base8453-pilot",
  ...
  const genesisInput = {
    ...
    stateRootSeed: hashJson("flowmemory.production_l1.state_root_seed.v0", { profileId: profile.profileId, chainId: profile.chainId }),
...
function buildGenesisFromInput(input) {
  const initialAccountsRoot = hashJson("flowmemory.production_l1.genesis_accounts_root.v0", input.initialAccounts);
  const initialBalancesRoot = hashJson("flowmemory.production_l1.genesis_balances_root.v0", input.initialBalances);
  const validatorSetRoot = hashJson("flowmemory.production_l1.genesis_validator_set_root.v0", input.validatorSet);
  const bridgePilotConfigHash = hashJson("flowmemory.production_l1.bridge_pilot_config.v0", input.bridgePilotConfig);
  const tokenDexBootstrapConfigHash = hashJson("flowmemory.production_l1.token_dex_bootstrap_config.v0", input.tokenDexBootstrapConfig);
  const genesisHashInput = {
    schema: "flowmemory.production_l1.genesis_hash_input.v0",
  ...
  return {
    ...
    genesisHashInput,
    genesisHash: hashJson("flowmemory.production_l1.genesis_hash.v0", genesisHashInput),
    productionReady: false
  };
}
...
function account(label, profile, roleFlags) {
  ...
  const accountId = hashJson("flowmemory.production_l1.account_id.v0", { label, publicKey, chainId: profile.chainId, networkProfile: profile.profileId });
  return {
    ...
    nonceDomain: {
      domainId: hashJson("flowmemory.production_l1.nonce_domain.v0", { accountId, chainId: profile.chainId, networkProfile: profile.profileId }),
      nextNonce: "1",
      replayScope: "chainId+networkProfile+genesisHash+accountId"
    },
    roleFlags,
    metadataHash: hashJson("flowmemory.production_l1.account_metadata.v0", { label, roleFlags }),
    status: "active",
    productionReady: false
  };
}
...
function publicKeyFor(label) {
  return `0x02${hashJson("flowmemory.production_l1.public_key_fixture.v0", { label }).slice(2)}`;
}

function addressFor(label) {
  return `0x${hashJson("flowmemory.production_l1.address_fixture.v0", { label }).slice(-40)}`;
}
...
function authority(account, authorityType, votingPower, finalityWeight) {
  return {
    ...
    authorityId: hashJson("flowmemory.production_l1.authority_id.v0", { accountId: account.accountId, authorityType }),
    ...
    votingPower,
    finalityWeight,
    metadataHash: hashJson("flowmemory.production_l1.authority_metadata.v0", { accountId: account.accountId, authorityType }),
    status: "active",
    productionReady: false
  };
}
...
function buildBridgeEvidence(genesis, accounts, nativeAssetId) {
  const depositCore = {
    ...
    sourceTxHash: hashJson("flowmemory.production_l1.source_tx_hash.v0", { name: "base-deposit-1" }),
  ...
  const observationId = hashJson("flowmemory.production_l1.bridge_observation_id.v0", sourceEventKey(depositCore));
  const creditId = hashJson("flowmemory.production_l1.bridge_credit_id.v0", {
    observationId,
  ...
  const releaseCore = {
    ...
    sourceNetwork: "base-mainnet-source-for-local-private-pilot",
    lockboxAddress: genesis.bridgePilotConfig.lockboxAddress,
    sourceTxHash: hashJson("flowmemory.production_l1.source_tx_hash.v0", { name: "base-release-1" }),
    ...
    finalityStatus: "source_finalized",
    release: {
      withdrawalIntentId: hashJson("flowmemory.production_l1.withdrawal_intent_id.v0", { accountId: accounts["bob-user"].accountId, amount: "100000" }),
      releaseTxHash: hashJson("flowmemory.production_l1.release_tx_hash.v0", { name: "base-release-1" }),
  ...
  const releaseObservationId = hashJson("flowmemory.production_l1.bridge_observation_id.v0", sourceEventKey(releaseCore));
  const releaseCreditId = hashJson("flowmemory.production_l1.bridge_credit_id.v0", {
...
function finishBridgeEvidence(evidence) {
  const duplicateKey = hashJson("flowmemory.production_l1.bridge_duplicate_key.v0", sourceEventKey(evidence));
  const evidenceHash = hashJson("flowmemory.production_l1.bridge_evidence_hash.v0", { ...evidence, duplicateKey });
  return {
    schema: BRIDGE_EVIDENCE_SCHEMA,
    evidenceId: hashJson("flowmemory.production_l1.bridge_evidence_id.v0", { evidenceHash }),
    ...evidence,
    duplicateKey,
    evidenceHash,
    localOnly: true,
...
function buildTransactions(genesis, profile, accounts, ids) {
  ...
  add("alice-user", "deployer", "token_launch", {
    ...
    tokenId: ids.tokenId,
    symbol: "FLOWT",
    name: "FlowMemory Local Test Token",
    decimals: 6,
  ...
  for (const payloadType of OBJECT_PAYLOAD_TYPES) {
    ...
    add("alice-user", "deployer", payloadType, {
      ...
      rootfieldId: hashJson("rootfield", "flowmemory-local-core"),
...
function makePayload(payloadType, actorAccountId, nonce, details, emittedEventTypes, balanceChanges) {
  const payloadId = hashJson("flowmemory.production_l1.payload_id.v0", { payloadType, actorAccountId, nonce: nonce.toString(), details });
  return {
...
function makeEnvelope(genesis, profile, account, role, payload) {
  const payloadHash = hashJson("flowmemory.production_l1.payload_hash.v0", payload);
  ...
  const signingDigest = hashJson("flowmemory.production_l1.signing_digest.v0", signingDigestInput);
  const txIdInput = {
    chainId: profile.chainId,
    networkProfile: profile.profileId,
    genesisHash: genesis.genesisHash,
  ...
  return {
    ...
    txId: hashJson("flowmemory.production_l1.tx_id.v0", txIdInput),
    protocolVersion: PROTOCOL_VERSION,
    chainId: profile.chainId,
    networkProfile: profile.profileId,
    genesisHash: genesis.genesisHash,
    ...
    signature: {
      ...
      signingDigest,
      value: `0x${signingDigest.slice(2)}${hashJson("flowmemory.production_l1.signature_tail.v0", signingDigest).slice(2)}`
    }
  };
}
...
function buildReceiptsAndEvents(transactions, bridgeEvidence) {
  ...
  for (const tx of transactions) {
    const receiptIdInput = hashJson("flowmemory.production_l1.receipt_id_input.v0", { txId: tx.txId, payloadType: tx.payloadType, status: "succeeded" });
    const receiptId = hashJson("flowmemory.production_l1.receipt_id.v0", { receiptIdInput });
    const txEvents = tx.payload.emittedEventTypes.map((eventType, index) => {
      const deterministicIdInput = hashJson("flowmemory.production_l1.event_id_input.v0", { txId: tx.txId, receiptId, eventType, index });
      return {
        schema: EVENT_SCHEMA,
        eventId: hashJson("flowmemory.production_l1.event_id.v0", { deterministicIdInput }),
        eventType,
    ...
    receipts.push({
      ...
      stateDeltaRef: hashJson("flowmemory.production_l1.state_delta_ref.v0", { txId: tx.txId, payloadType: tx.payloadType }),
...
function buildStateRootManifest(genesis, transactionSet, receiptsAndEvents, bridgeEvidence, ids) {
  const stateComponents = {
    ...
    tokens: [
      { tokenId: ids.tokenId, symbol: "FLOWT", name: "FlowMemory Local Test Token", decimals: 6, totalSupply: "1000250000", noValue: true }
    ],
    pools: [
  ...
  const components = STATE_COMPONENTS.map((component) => {
    ...
    return {
      component,
      root: hashJson("flowmemory.production_l1.state_component_root.v0", { component, entries }),
      count: entries.length,
      ownerAgent: ownerForStateComponent(component),
      inputs: stateInputsForComponent(component)
    };
  });
  const stateRoot = hashJson("flowmemory.production_l1.state_root.v0", {
  ...
  return {
    ...
    deterministicReplay: {
      ...
      replayCommand: "npm run validate:production-network-fixtures"
    }
  };
}
...
function buildBlock(genesis, profile, accounts, transactions, receipts, events, bridgeEvidence, stateRootManifest) {
  const txRoot = hashJson("flowmemory.production_l1.tx_root.v0", transactions.map((tx) => tx.txId));
  const receiptRoot = hashJson("flowmemory.production_l1.receipt_root.v0", receipts.map((receipt) => receipt.receiptId));
  const eventRoot = hashJson("flowmemory.production_l1.event_root.v0", events.map((event) => event.eventId));
  const evidenceRoot = hashJson("flowmemory.production_l1.evidence_root.v0", bridgeEvidence.map((evidence) => evidence.evidenceId));
  const headerBase = {
  ...
  const header = {
    ...headerBase,
    blockHash: hashJson("flowmemory.production_l1.block_hash.v0", headerBase)
  };
  return {
...
function buildFinalityReceipt(genesis, profile, accounts, block) {
  const certificateHash = hashJson("flowmemory.production_l1.finality_certificate_hash.v0", {
  ...
  return {
    ...
    finalityReceiptId: hashJson("flowmemory.production_l1.finality_receipt_id.v0", { blockHash: block.header.blockHash, certificateHash }),
    chainId: genesis.chainId,
    networkProfile: genesis.networkProfile,
    ...
    status: "accepted",
    validatorSetHash: hashJson("flowmemory.production_l1.validator_set_hash.v0", genesis.validatorSet),
    certificateHash,
...
function buildExportSnapshot(genesis, block, stateRootManifest, finalityReceipt, ids) {
  return {
    ...
    provenance: {
      generatedBy: "fixtures/production-network/production-network-tools.mjs",
      schemaNames: Object.keys(buildSchemas()).sort(),
      fixturePaths: [
        "fixtures/production-network/profiles.json",
        "fixtures/production-network/genesis.input.json",
        "fixtures/production-network/genesis.json",
        "fixtures/production-network/transactions.valid.json",
        "fixtures/production-network/receipts.valid.json",
        "fixtures/production-network/block.valid.json",
        "fixtures/production-network/negative-fixtures.json"
      ],
      validationCommands: [
        "npm run validate:production-network-protocol",
        "npm run validate:production-network-fixtures"
      ]
    },
    productionReady: false
  };
...
function buildNegativeFixtures(genesis, profile, transactions, stateRootManifest, bridgeEvidence) {
  ...
  return {
    schema: "flowmemory.production_l1.negative_fixture_set.v0",
    cases: [
      {
...
function writeSchemasAndFixtures() {
  ...
  const all = buildAll();
  writeJson(resolve(fixturesDir, "profiles.json"), { schema: "flowmemory.production_l1.network_profile_set.v0", profiles: all.profiles });
  writeJson(resolve(fixturesDir, "genesis.input.json"), all.genesisInput);
  writeJson(resolve(fixturesDir, "genesis.json"), all.genesis);
  writeJson(resolve(fixturesDir, "transactions.valid.json"), { schema: "flowmemory.production_l1.transaction_fixture_set.v0", transactions: all.transactions });
  writeJson(resolve(fixturesDir, "receipts.valid.json"), { schema: "flowmemory.production_l1.receipt_fixture_set.v0", receipts: all.receipts });
  writeJson(resolve(fixturesDir, "events.valid.json"), { schema: "flowmemory.production_l1.event_fixture_set.v0", events: all.events });
  writeJson(resolve(fixturesDir, "bridge-evidence.valid.json"), { schema: "flowmemory.production_l1.bridge_evidence_fixture_set.v0", bridgeEvidence: all.bridgeEvidence });
...
async function validateProtocol() {
  ...
  for (const [name, schema] of Object.entries(schemas)) {
    ...
    if (canonicalJson(disk) !== canonicalJson(schema)) {
      throw new Error(`schema drift: ${name}; run node fixtures/production-network/production-network-tools.mjs write`);
    }
...
async function validateFixtures() {
  ...
  const fixtureComparisons = [
    ["profiles.json", { schema: "flowmemory.production_l1.network_profile_set.v0", profiles: rebuilt.profiles }, profileSet],
    ["genesis.input.json", rebuilt.genesisInput, genesisInput],
    ["genesis.json", rebuilt.genesis, genesis],
    ["transactions.valid.json", { schema: "flowmemory.production_l1.transaction_fixture_set.v0", transactions: rebuilt.transactions }, readJson(resolve(fixturesDir, "transactions.valid.json"))],
    ["receipts.valid.json", { schema: "flowmemory.production_l1.receipt_fixture_set.v0", receipts: rebuilt.receipts }, readJson(resolve(fixturesDir, "receipts.valid.json"))],
    ["events.valid.json", { schema: "flowmemory.production_l1.event_fixture_set.v0", events: rebuilt.events }, readJson(resolve(fixturesDir, "events.valid.json"))],
    ["bridge-evidence.valid.json", { schema: "flowmemory.production_l1.bridge_evidence_fixture_set.v0", bridgeEvidence: rebuilt.bridgeEvidence }, readJson(resolve(fixturesDir, "bridge-evidence.valid.json"))],
  ...
  for (const [name, expected, actual] of fixtureComparisons) {
    if (canonicalJson(expected) !== canonicalJson(actual)) {
      throw new Error(`fixture drift: ${name}; run node fixtures/production-network/production-network-tools.mjs write`);
    }
  }
...
function validateTransaction(tx, genesis, profile, accountMap, options = {}) {
  ...
  const payloadHash = hashJson("flowmemory.production_l1.payload_hash.v0", tx.payload);
  if (tx.payloadHash !== payloadHash) return ERROR_CODES.MALFORMED_PAYLOAD_HASH;
  const txId = hashJson("flowmemory.production_l1.tx_id.v0", {
    chainId: tx.chainId,
    networkProfile: tx.networkProfile,
    genesisHash: tx.genesisHash,
    nonce: tx.nonce,
...
function validateStateRootManifest(manifest) {
  const components = manifest.components.map((component) => {
    ...
    const root = hashJson("flowmemory.production_l1.state_component_root.v0", { component: component.component, entries });
...
function validateBlock(block, transactions, receipts, events, bridgeEvidence, stateRootManifest) {
  ...
  const blockHash = hashJson("flowmemory.production_l1.block_hash.v0", headerBase);
  if (blockHash !== block.header.blockHash) throw new Error(`block hash mismatch: ${block.header.blockHash}`);
  const roots = {
    txRoot: hashJson("flowmemory.production_l1.tx_root.v0", transactions.map((tx) => tx.txId)),
    receiptRoot: hashJson("flowmemory.production_l1.receipt_root.v0", receipts.map((receipt) => receipt.receiptId)),
    eventRoot: hashJson("flowmemory.production_l1.event_root.v0", events.map((event) => event.eventId)),
    evidenceRoot: hashJson("flowmemory.production_l1.evidence_root.v0", bridgeEvidence.map((evidence) => evidence.evidenceId))
  };
...
async function main() {
  ...
  if (command === "write") {
    ...
    console.log(`FLOWMEMORY_PRODUCTION_NETWORK_WRITE_OK schemas=${Object.keys(buildSchemas()).length} transactions=${all.transactions.length} negativeCases=${all.negativeFixtures.cases.length}`);
    return;
  }
  if (command === "build-genesis") {
    const all = buildAll();
    writeJson(resolve(fixturesDir, "genesis.json"), all.genesis);
    console.log(`FLOWMEMORY_PRODUCTION_NETWORK_GENESIS_BUILD_OK genesisHash=${all.genesis.genesisHash}`);
    return;
  }
  if (command === "validate-genesis") {
    ...
    console.log(`FLOWMEMORY_PRODUCTION_NETWORK_GENESIS_OK genesisHash=${genesis.genesisHash}`);
    return;
  }
  ...
  if (command === "validate-protocol") {
    ...
    console.log(`FLOWMEMORY_PRODUCTION_NETWORK_PROTOCOL_OK schemas=${result.schemas} profiles=${result.profiles} payloadTypes=${result.payloadTypes}`);
    return;
  }
  if (command === "validate-fixtures") {
    const result = await validateFixtures();
    console.log(`FLOWMEMORY_PRODUCTION_NETWORK_FIXTURES_OK transactions=${result.transactions} receipts=${result.receipts} events=${result.events} bridgeEvidence=${result.bridgeEvidence} negativeCases=${result.negativeCases} genesisHash=${result.genesisHash} stateRoot=${result.stateRoot}`);
    return;
  }
  console.log("usage: node fixtures/production-network/production-network-tools.mjs <write|build-genesis|validate-genesis|genesis-hash|validate-protocol|validate-fixtures>");
}
