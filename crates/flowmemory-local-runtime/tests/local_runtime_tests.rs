use flowmemory_local_runtime::model::{
    BASE_MAINNET_CHAIN_ID, BRIDGE_PILOT_ACCOUNT_OWNER, BRIDGE_RUNTIME_AMOUNT_STORAGE,
    BridgeConfirmationProof, BridgePilotCapProof, LocalRuntimeError, FLOWPULSE_TOPIC0,
...
use flowmemory_local_runtime::{canonical_json, keccak_hex};
...
fn invalid_dependencies_are_rejected() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ModelPassportMissing(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ArtifactMissing(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ArtifactMissing("artifact:missing".to_string()))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::VerifierModuleMissing(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::WorkReceiptMissing(
...
fn every_core_transaction_type_can_be_applied() {
    ...
    queue_transaction(
        ...
        Transaction::ImportFlowPulseObservation(
            flowmemory_local_runtime::model::ImportedFlowPulseObservation {
    ...
    queue_transaction(
        &mut state,
        Transaction::ImportVerifierReport(flowmemory_local_runtime::model::ImportedVerifierReport {
...
fn local_faucet_and_transfer_update_test_unit_ledger() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::LocalTestUnitBalanceInsufficient(
...
fn rejected_local_transfer_does_not_debit_sender_when_recipient_is_missing() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::LocalTestUnitBalanceMissing(
...
fn pilot_bridge_credit_maps_asset_account_and_rejects_replay() {
    ...
    let account_id = PILOT_FLOWMEMORY_RECIPIENT.to_string();
    assert_eq!(
...
fn live_bridge_credit_applies_exact_amount_and_rejects_replay() {
    ...
    assert_eq!(
        replay,
        Err(LocalRuntimeError::BridgeCreditReplayAlreadyConsumed(
            replay_key.clone()
        ))
    );
...
fn live_bridge_credit_rejects_wrong_chain_unsupported_asset_zero_and_bad_identity() {
    ...
    assert_rejected_without_state_change(
        &mut wrong_chain,
        &wrong_chain_tx,
        LocalRuntimeError::BridgeCreditWrongSourceChain("84532".to_string()),
    );
    ...
    assert_rejected_without_state_change(
        ...
        LocalRuntimeError::BridgeCreditUnsupportedAsset(UNSUPPORTED_LIVE_SOURCE_TOKEN.to_string()),
    );
    ...
    assert_rejected_without_state_change(
        &mut zero_amount,
        &zero_amount_tx,
        LocalRuntimeError::BridgeCreditAmountMustBePositive(zero_credit_id),
    );
    ...
    assert!(matches!(
        ...
        Err(LocalRuntimeError::DeterministicIdMismatch { kind, .. }) if kind == "bridge credit"
    ));
...
fn live_bridge_credit_rejects_unsatisfied_confirmation_and_pilot_cap_excess() {
    ...
    assert_rejected_without_state_change(
        &mut unsatisfied,
        &unsatisfied_tx,
        LocalRuntimeError::BridgeCreditConfirmationUnsatisfied(unsatisfied_credit_id),
    );
    ...
    assert_rejected_without_state_change(
        &mut above_cap,
        &above_cap_tx,
        LocalRuntimeError::BridgeCreditAmountExceedsPilotCap(above_cap_credit_id),
    );
}
  ...
  "credits": [
    {{
      ...
      "productionReady": true
            ...
            LIVE_BRIDGE_TX_HASH,
            LIVE_SOURCE_TOKEN,
            LIVE_FLOWMEMORY_RECIPIENT
        ),
    )
    .expect("write overflow handoff");

    let status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
...
fn live_bridge_credit_survives_export_import_restart_and_remains_transferable() {
    ...
    let mut imported: flowmemory_local_runtime::model::ChainState =
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::BridgeCreditReplayAlreadyConsumed(replay_key))
    );
    apply_transaction(
        &mut imported,
        &Transaction::CreateLocalTestUnitBalance {
...
fn token_and_dex_reject_duplicate_zero_insufficient_and_slippage_failures() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::TokenAlreadyExists(token_id.clone()))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::PoolAlreadyExists(pool_id.clone()))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::TokenAmountMustBePositive(zero_liquidity_id))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::SwapSlippageExceeded(slippage_swap_id))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::PoolMissing("pool:missing".to_string()))
    );
}
...
fn duplicate_ids_are_rejected_for_new_objects() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ModelPassportAlreadyExists(
            "model:dup".to_string()
        ))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::AgentAlreadyExists("agent:dup".to_string()))
    );
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::LocalTestUnitBalanceAlreadyExists(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::FaucetRecordAlreadyExists(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::VerifierModuleAlreadyExists(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ArtifactAvailabilityAlreadyExists(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ChallengeAlreadyExists(
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::FinalityReceiptAlreadyExists(
...
fn local_test_unit_faucet_updates_balance_without_value_claims() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::FaucetAmountMustBePositive(
...
fn memory_update_rejects_missing_or_failed_source_receipt() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::WorkReceiptFailed("receipt:status".to_string()))
    );
}
...
fn finalization_rejects_unresolved_challenge() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::ChallengeUnresolved(
...
fn finalization_rejects_invalid_finality_status() {
    ...
    assert_eq!(
        ...
        Err(LocalRuntimeError::InvalidFinalityStatus("pending".to_string()))
    );
}
...
fn cli_demo_writes_state_and_handoff_files() {
    ...
    let status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        ...
        .status()
        .expect("run flowmemory-local-runtime");
    assert!(status.success());
...
fn cli_smoke_runs_full_flow() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "smoke",
...
fn cli_start_runs_10_blocks_and_state_survives_restart() {
    ...
    let first = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "start",
    ...
    let inspect = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "inspect-state",
    ...
    let second = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "start",
    ...
    let inspect_again = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "inspect-state",
...
fn cli_export_import_state_round_trip_is_deterministic() {
    ...
    let demo_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "demo",
    ...
    let export_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "export-state",
    ...
    let import_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
...
fn cli_pilot_bridge_handoff_receipts_survive_restart_and_export_import() {
    ...
    let product_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "product-smoke",
    ...
    let bridge_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "bridge-handoff",
    ...
    let block_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args(["--state", state.to_str().expect("state path"), "run-block"])
        .status()
        .expect("include bridge handoff");
    assert!(block_status.success());

    let by_id = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "bridge-receipt",
    ...
    let by_wrong_id = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "bridge-receipt",
    ...
    let by_event = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "bridge-receipt",
    ...
    let by_wrong_event = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "bridge-receipt",
    ...
    let restarted = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "start",
    ...
    let export_handoff_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "export-fixtures",
...
fn cli_node_runs_ten_blocks_and_includes_authorized_inbox_tx() {
    ...
    let faucet = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "--node-dir",
    ...
    let node = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "--node-dir",
...
fn cli_submit_tx_batches_multiple_txs_and_preserves_order() {
    ...
    let fixture_json = serde_json::json!({
        "schema": "flowmemory.local_runtime.ordered_submit_tx_fixture.v0",
        "txs": [
            {
    ...
    let submit = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "--node-dir",
    ...
    let tick = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state.to_str().expect("state path"),
            "--node-dir",
...
fn cli_static_peer_sync_reconciles_two_local_node_states() {
    ...
    let node_a_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
            state_a.to_str().expect("state a"),
            "--node-dir",
    ...
    std::fs::write(
        &peer_b,
        format!(
            "{{\"schema\":\"flowmemory.local_runtime.static_peers.v0\",\"nodeId\":\"node:test:b\",\"peers\":[{{\"nodeId\":\"node:test:a\",\"statePath\":\"{}\"}}]}}\n",
            state_a.to_string_lossy().replace('\\', "\\\\")
        ),
    )
    .expect("write peer config");

    let node_b_status = Command::new(env!("CARGO_BIN_EXE_flowmemory-local-runtime"))
        .args([
            "--state",
...
const PILOT_FLOWMEMORY_RECIPIENT: &str =
    "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
const PILOT_OBSERVATION_ID: &str =
    "0xfd0f7526f3c410fcb5e3482cee7beeaae9728b2656c9fd48c55fce6dca639ef2";
const PILOT_DEPOSIT_ID: &str = "0x7e3a7f7ab7dc9b07d762c1f2fce315cf0c08f1a7e854b4dbcb2359efcb9cb269";
const PILOT_REPLAY_KEY: &str = "0x4224dacd2bf62efb96d8e9a7f6e63197b3be489a086e581a0b80c5c3d06ff3c2";
const PILOT_BRIDGE_CREDIT_ID: &str =
    "0x178162bda383ada7811bdb2f2250031505dcb34c6b49d7f0faffecce6c2249f7";
...
const LIVE_FLOWMEMORY_RECIPIENT: &str =
    "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef";
...
fn pilot_bridge_setup_and_credit_txs() -> Vec<Transaction> {
    ...
    let account_mapping_id =
        deterministic_bridge_account_mapping_id(PILOT_FLOWMEMORY_RECIPIENT, &account_id);
    ...
    vec![
        ...
        Transaction::MapBridgeAccount {
            ...
            flowmemory_recipient: PILOT_FLOWMEMORY_RECIPIENT.to_string(),
...
fn setup_live_bridge_account(
    state: &mut flowmemory_local_runtime::model::ChainState,
    source_chain_id: &str,
    source_token: &str,
) -> String {
    let account_id = LIVE_FLOWMEMORY_RECIPIENT.to_string();
    let asset_mapping_id = deterministic_bridge_asset_mapping_id(
        source_chain_id,
        source_token,
        LOCAL_TEST_UNIT_ASSET_ID,
    ...
    let account_mapping_id =
        deterministic_bridge_account_mapping_id(LIVE_FLOWMEMORY_RECIPIENT, &account_id);
    apply_transaction(
        state,
        &Transaction::MapBridgeAsset {
            mapping_id: asset_mapping_id,
    ...
    apply_transaction(
        ...
        &Transaction::MapBridgeAccount {
            mapping_id: account_mapping_id,
            flowmemory_recipient: LIVE_FLOWMEMORY_RECIPIENT.to_string(),
            account_id: account_id.clone(),
            owner: BRIDGE_PILOT_ACCOUNT_OWNER.to_string(),
            local_only: false,
            production_ready: true,
...
fn live_bridge_credit_tx(
    ...
    Transaction::CreditBridgeFromBaseEvent {
        ...
        pilot_cap_proof: Some(BridgePilotCapProof {
            ...
            pilot_mode_tag: Some(
                "0xefaa6123a077975f7e3f91a9408b025c8eaf4422c372add208d3d0c5d8ce8f92".to_string(),
            ),
...
fn temp_dir(name: &str) -> std::path::PathBuf {
    let temp = std::env::temp_dir().join(format!(
        "flowmemory-local-runtime-test-{}-{name}",
        std::process::id()
    ));
    if temp.exists() {
...
fn run_demo_chain() -> (
    ...
    String,
    String,
    flowmemory_local_runtime::model::StateMapRoots,
) {
    let mut state = genesis_state();
    for tx in demo_transactions() {
        queue_transaction(&mut state, tx);
...
fn setup_product_test_accounts(state: &mut flowmemory_local_runtime::model::ChainState) {
    apply_transaction(
        state,
