...
const SECRET_PATTERNS: RegExp[] = [
  ...
  /\b(private[_ -]?key|seed[_ -]?phrase|mnemonic|api[_ -]?key|webhook|bearer|token|password|passphrase|vault[_ -]?ciphertext)\b\s*[:=]\s*["']?[^"'\s,}]+/gi,
...
export function redactFlowChainText(value: unknown): string {
  ...
  for (const pattern of SECRET_PATTERNS) {
    text = text.replace(pattern, (match) => {
      if (/^0x[a-fA-F0-9]{64}$/.test(match)) return "[REDACTED_32_BYTE_HEX]";
...
export function redactJsonValue(value: JsonValue): JsonValue {
  ...
  if (value !== null && typeof value === "object") {
    ...
    for (const [key, entry] of Object.entries(value)) {
      if (key !== "noSecrets" && /private|secret|seed|mnemonic|password|passphrase|token|apiKey|api_key|webhook|vault/i.test(key)) {
