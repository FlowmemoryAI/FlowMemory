...
test("owner inputs validator fails for placeholder template", () => {
  ...
  assert.match(result.stderr, /must be a real deployed address/);
  assert.match(result.stderr, /must be a real operator address/);
  assert.match(result.stderr, /must be filled with a real person or firm/);
});

test("owner inputs validator passes for synthetic real values", () => {
  const dir = mkdtempSync(join(tmpdir(), "flowmemory-agent-bonds-owner-inputs-"));
  const configPath = join(dir, "owner-inputs.json");
  ...
  assert.match(result.stdout, /flowmemory-agent-bonds-owner-inputs-validate/);
});
