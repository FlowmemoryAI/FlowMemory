...
| Operations | Node operator package verifier independently checks generated package files, command matrix, owner-input name-only boundary, forbidden local files, and no-secret scan. | passed | operatorPackageVerify=passed, expectedFiles=62, commands=51, failedChecks=0, noSecretScan=True |
